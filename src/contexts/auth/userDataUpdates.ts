...
export const updateUserData = async (userData: UserData): Promise<boolean> => {
  try {
    ...
    // CRITICAL: We need to explicitly update the profile_completed flag when the user intends to 
    // mark their profile as completed. This is essential for setup progress tracking.
    ...
    if (userType === 'developer') {
      ...
      if (hasProperty(userData, 'languagesSpoken')) specificProfileData.languages_spoken = userData.languagesSpoken;
